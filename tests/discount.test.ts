...
test('discounts are server priced, private, validated and retry safe',async()=>{
 const db=new PGlite();
 try {
 ...
 for(const f of ['202609150001_bookings.sql','202609180001_discounts.sql']) await db.exec(await readFile(new URL('../supabase/migrations/'+f,import.meta.url),'utf8'));
 ...
 const paid=await db.query<{r:{price:number}}>(sql,[slots[1].id,...args.slice(1,8),'44444444-4444-4444-8444-444444444444',null]);assert.equal(paid.rows[0].r.price,120);
